...
from utils.logo_detection import Detection as Logo_Detection
from utils.camera_no_thread import Camera
...
class Calculate_Change:
    ...
    def init_ref(self,img=None,ref=[],flag=False):
        if flag:
            coors = detector.Object_Detection(img)
            try:
                self._refrence_list = coors[0][1]
            except IndexError:
                pass
        else:
            self._refrence_list = ref
...
class Execution:
    ...
    def Run(self):
        ...
        # while not self._cam.exit_signal.is_set():
            ...
            new_logo = self._detector.Object_Detection(frame)
            try:
                new_logo = new_logo[0][1]
            except IndexError: 
                new_logo = []
            ...
            if not self._change_detector.refrence_list:
                self._change_detector.init_ref(img=frame)
            
            if (counter % 100 == 0) and (counter != 0) and new_logo and self._change_detector.refrence_list:
                self._change_detector.Algorithm(new_logo)
